...
// Types for Inbound call data (mapped from API)
...
function determinePurpose(agentName: string, direction: string): CallPurpose {
  ...
  if (direction.includes('outbound')) {
    return 'sales'; // Default Inbound calls to sales
  }
  return 'other';
}
...
export default function OutboundCallsPage() {
    ...
    const loadCalls = async () => {
      try {
        ...
        if (response.data.success) {
          // Filter only Inbound calls and map to our format
          const outboundCalls = response.data.calls
            .filter(call => call.direction_badge !== 'Outbound' && call.direction_badge !== '📞 Outbound' && call.direction_badge !== 'outbound_api')
            .map(mapApiCallToOutboundCall);
          
          setCalls(outboundCalls);
          setTotalCalls(response.data.total);
          
          // Calculate total pages based on Inbound calls
  ...
  const handleNewCall = () => {
    console.log('Creating new Inbound call');
    alert('New Inbound call feature - would open call creation form');
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {error && (
          <div className="mb-6 rounded-lg border border-red-300 dark:border-red-500/30 bg-red-50 dark:bg-red-500/10 p-4 text-sm">
            <p className="font-medium text-red-800 dark:text-red-100">Failed to load Inbound calls</p>
            ...
            <table className="w-full">
              ...
              <tbody className="text-sm">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                      {calls.length === 0 ? 'No Inbound calls found.' : 'No calls found matching your criteria.'}
                    </td>
                  </tr>
                ) : (
